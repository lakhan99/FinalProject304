...
Finally, Figure 5 outlines the impact of having family history with obesity. We see that of overwight and obese population, those who have a family history with obesity are much more likely to be obese and overweight compared to those without obesity in their family. This is informative of the role of genetics in determining one's BMI. 

```{r, fig.cap="Family history with obesity in proportion to population with obesity", fig.height=3}
...
From Figure 5, the true relationship between BMI and the fitted values of the model seem to be linear while the residual plots show randomness which is ideal. This means that the uncorrelated errors and constant variance assumption is also satisfied. One thing to note is that the residual plot for the Age variable is also skewed right, something that we noticed when visualizing our data. Finally, since a majority of the points align very closely with the line, the normality distribution is also satisfied.      

```{r, fig.cap= "Response vs Fitted Values", fig.height= 3, message= FALSE}
...
```{r, fig.cap="Residuals versus Fitted Values to check uncorrelated errors and constant variance assumption", fig.height=3}
#Checking residual plots 
res_plot1 <- plot(res_final ~ fitted(model_final), xlab= "Fitted Values", ylab = "Residuals")
```

```{r, fig.cap="Residuals versus Age to check uncorrelated errors and constant variance assumption", fig.height= 3}
res_plot2<- plot(res_final ~ train$Age, xlab= "Age", ylab = "Residuals")
```


```{r fig.cap= "QQ plot to check normality assumptions", fig.height= 3}
